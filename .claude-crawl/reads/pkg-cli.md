[Skip to main content](https://hyperframes.heygen.com/packages/cli#content-area)
...
## [​](https://hyperframes.heygen.com/packages/cli\#when-to-use)  When to Use
...
- Render programmatically from Node.js code — use the [producer](https://hyperframes.heygen.com/packages/producer)
- Build a custom frame capture pipeline — use the [engine](https://hyperframes.heygen.com/packages/engine)
- Embed a composition editor in your own web app — use the [studio](https://hyperframes.heygen.com/packages/studio)
- Parse or generate composition HTML in code — use [core](https://hyperframes.heygen.com/packages/core)
...
## [​](https://hyperframes.heygen.com/packages/cli\#agent-friendly-by-default)  Agent-Friendly by Default
...
### [​](https://hyperframes.heygen.com/packages/cli\#json-output-and-_meta-envelope)  JSON Output and `_meta` Envelope
...
### [​](https://hyperframes.heygen.com/packages/cli\#passive-update-notices)  Passive Update Notices
...
## [​](https://hyperframes.heygen.com/packages/cli\#getting-started)  Getting Started

1

[Navigate to header](https://hyperframes.heygen.com/packages/cli#)
...
See [Examples](https://hyperframes.heygen.com/examples) for all available examples.
...
## [​](https://hyperframes.heygen.com/packages/cli\#commands)  Commands
...
### [​](https://hyperframes.heygen.com/packages/cli\#init)  `init`
...
In default (agent) mode, `--example` is required — the CLI errors with a usage example if missing. In `--human-friendly` mode, you choose interactively. When `--video` or `--audio` is provided, the CLI automatically transcribes the audio with Whisper and patches captions into the composition (use `--skip-transcribe` to disable).After scaffolding, the CLI installs AI coding skills for Claude Code, Gemini CLI, and Codex CLI (use `--skip-skills` to disable). See [`skills`](https://hyperframes.heygen.com/packages/cli#skills) command.See [Examples](https://hyperframes.heygen.com/examples) for full details.

### [​](https://hyperframes.heygen.com/packages/cli\#add)  `add`

Install a **block** or **component** from the registry into an existing project. Examples (full projects) are scaffolded with [`init`](https://hyperframes.heygen.com/packages/cli#init); blocks and components are smaller units you add to a composition you already have.
...
### [​](https://hyperframes.heygen.com/packages/cli\#compositions)  `compositions`
...
### [​](https://hyperframes.heygen.com/packages/cli\#transcribe)  `transcribe`
...
### [​](https://hyperframes.heygen.com/packages/cli\#tts)  `tts`
...
### [​](https://hyperframes.heygen.com/packages/cli\#capture)  `capture`
...
The capture command extracts everything an AI agent needs to understand a website’s visual identity: viewport screenshots at every scroll depth, color palette (pixel-sampled + DOM computed), font files, images with semantic names, SVGs, Lottie animations, video previews, WebGL shaders, visible text, and page structure.Output is a self-contained directory with a `CLAUDE.md` file that any AI agent can read to understand the captured site. Used by the `/website-to-hyperframes` skill as step 1 of the video production pipeline.Set `GEMINI_API_KEY` in a `.env` file for AI-powered image descriptions via Gemini vision (~$0.001/image). See the [Website to Video](https://hyperframes.heygen.com/guides/website-to-video#enriching-captures-with-gemini-vision) guide for details.

### [​](https://hyperframes.heygen.com/packages/cli\#preview)  `preview`
...
Visual output matches render exactly. Playback _performance_ does not: preview plays in real time in your browser, so paint-heavy compositions (large images, stacked `backdrop-filter` layers, many shadowed elements) may stutter depending on your hardware. The rendered mp4 is always accurate regardless — render captures frames one at a time, so per-frame cost shows up as longer render duration, not dropped frames. See [Performance](https://hyperframes.heygen.com/guides/performance) for details.
...
### [​](https://hyperframes.heygen.com/packages/cli\#lint)  `lint`
...
The linter detects missing attributes, missing adapter libraries (GSAP, Lottie, Three.js), structural problems, and more. See [Common Mistakes](https://hyperframes.heygen.com/guides/common-mistakes) for details on each rule.

### [​](https://hyperframes.heygen.com/packages/cli\#snapshot)  `snapshot`
...
The snapshot command bundles the project, serves it locally, launches headless Chrome, seeks to each timestamp, and captures a 1920×1080 PNG. Useful for visual verification during the build step of the [website-to-video](https://hyperframes.heygen.com/guides/website-to-video) workflow.

### [​](https://hyperframes.heygen.com/packages/cli\#render)  `render`
...
| `--hdr` | — | off | Detect HDR sources and output HDR10 (H.265 10-bit, BT.2020 PQ/HLG). MP4 only. SDR-only compositions are unaffected. See [HDR Rendering](https://hyperframes.heygen.com/guides/hdr) |
...
#### [​](https://hyperframes.heygen.com/packages/cli\#webm-with-transparency)  WebM with Transparency
...
See [Rendering](https://hyperframes.heygen.com/guides/rendering) for all options and modes.

### [​](https://hyperframes.heygen.com/packages/cli\#benchmark)  `benchmark`
...
### [​](https://hyperframes.heygen.com/packages/cli\#doctor)  `doctor`
...
### [​](https://hyperframes.heygen.com/packages/cli\#info)  `info`
...
### [​](https://hyperframes.heygen.com/packages/cli\#upgrade)  `upgrade`
...
### [​](https://hyperframes.heygen.com/packages/cli\#browser)  `browser`
...
### [​](https://hyperframes.heygen.com/packages/cli\#docs)  `docs`
...
### [​](https://hyperframes.heygen.com/packages/cli\#telemetry)  `telemetry`
...
### [​](https://hyperframes.heygen.com/packages/cli\#skills)  `skills`
...
#### [​](https://hyperframes.heygen.com/packages/cli\#troubleshooting-fatal-active-post-checkout-hook-found-during-git-clone)  Troubleshooting: `fatal: active post-checkout hook found during git clone`
...
## [​](https://hyperframes.heygen.com/packages/cli\#hyperframes-json)  hyperframes.json
...
## [​](https://hyperframes.heygen.com/packages/cli\#related-packages)  Related Packages
...
[Previous\\
\\
@hyperframes/studioVisual composition editor with live preview, timeline view, and hot reload.](https://hyperframes.heygen.com/packages/studio)

⌘I

On this page

- [When to Use](https://hyperframes.heygen.com/packages/cli#when-to-use)
- [Agent-Friendly by Default](https://hyperframes.heygen.com/packages/cli#agent-friendly-by-default)
- [JSON Output and \_meta Envelope](https://hyperframes.heygen.com/packages/cli#json-output-and-_meta-envelope)
- [Passive Update Notices](https://hyperframes.heygen.com/packages/cli#passive-update-notices)
- [Getting Started](https://hyperframes.heygen.com/packages/cli#getting-started)
- [Commands](https://hyperframes.heygen.com/packages/cli#commands)
- [hyperframes.json](https://hyperframes.heygen.com/packages/cli#hyperframes-json)
- [Related Packages](https://hyperframes.heygen.com/packages/cli#related-packages)

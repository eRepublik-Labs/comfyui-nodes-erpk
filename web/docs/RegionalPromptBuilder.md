...
- **Right-click a region** opens its detail view above the list: a mask thumbnail, an editable layer **name**, the X/Y/W/H pixel fields, the region **prompt**, an eye toggle that hides just that region, and a delete button. **Right-click empty canvas** opens just the list (top = front): click selects, drag rows to reorder depth, and each row leads with an eye toggle before its color, with duplicate and delete on the right. The eye in the status strip hides or shows every region overlay at once (same as **H**).

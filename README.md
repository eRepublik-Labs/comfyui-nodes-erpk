...
- Object scan: with an image connected, a ✦ button in the canvas corner runs a fully local detect-segment-depth pipeline (Florence-2 + SAM + Depth-Anything; weights download once on the first scan) that turns the prominent objects into real editable regions - labels become descriptions, same-class objects share a color family, measured depth orders the scene back-to-front, and segmentation masks render as a toggleable overlay. Shift-click scans with Gemini in the cloud instead

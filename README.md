...
- Object scan: with an image connected, a ✦ button in the canvas corner has Gemini detect the scene's objects - each becomes a real editable region with a layer name (the label) and a per-region prompt (a one-sentence caption that feeds generation), depth pre-ranked back-to-front, while SAM computes segmentation masks locally (weights download once). The scan replaces the canvas as a single undo step. A ⚙ options window picks the Gemini model (saved with the workflow)
...
- Object move/scale: drag a scanned object and its masked cut-out follows live with a dashed ghost at the origin; at execute time the move is composited into the image output deterministically, and the prompt only asks the edit model to remove the leftover duplicate and blend the pasted copy

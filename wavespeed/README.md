...
| **MiniMax H3 LoRA Stack** | Config node: up to 3 `{path, scale}` LoRA weights. Connect to the Text-to-Image or Image Edit `loras` socket | - | - | Free (no API call) | - |
...
| **MiniMax H3 Text-to-Video** | MiniMax H3 (MiniMax-hosted) | Picture and native stereo audio in one pass; audio steered by an `Audio:` line in the prompt. No API seed | 4-15s | 16:9/9:16/1:1/4:3/3:4/21:9 · 768p/2k | [Link](https://wavespeed.ai/models/minimax/h3/text-to-video) |
| **MiniMax H3 Image-to-Video** | MiniMax H3 (MiniMax-hosted) | Animate a first frame, optionally interpolating toward a last frame. Canvas follows the first image. No API seed | 4-15s | 768p/2k | [Link](https://wavespeed.ai/models/minimax/h3/image-to-video) |
| **MiniMax H3 Reference-to-Video** | MiniMax H3 (MiniMax-hosted) | Up to 9 reference images, 3 videos, 3 audios, each cited in the prompt by `<Picture N>` / `<Video N>` / `<Audio N>` tags. Accepts IMAGE, VIDEO and AUDIO inputs directly, or URLs. No API seed | 4-15s | 16:9/9:16/1:1/4:3/3:4/21:9 · 768p/2k | [Link](https://wavespeed.ai/models/minimax/h3/reference-to-video) |
| **MiniMax H3 Video Edit** | MiniMax H3 (open weights) | Rewrite lighting, style, environment or elements of a clip; up to 9 reference images and 3 audios; takes a VIDEO input or chains off any node's video_url. Duration and aspect follow the input unless set | follows input or 3-15s | 480p/540p/768p/1080p | [Link](https://wavespeed.ai/models/wavespeed-ai/minimax-h3/video-edit) |
| **MiniMax H3 Video Extend** | MiniMax H3 (open weights) | Append a new segment after a clip's last frame, optionally toward a target end frame; takes a VIDEO input or chains off any node's video_url | 3-15s new segment | 480p/540p/768p/1080p | [Link](https://wavespeed.ai/models/wavespeed-ai/minimax-h3/video-extend) |
